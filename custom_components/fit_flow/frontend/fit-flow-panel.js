...
class FitFlowPanel extends HTMLElement {
  ...
  configView() {
    return `<div class="grid">
      ...
      <section class="card"><h2>Exercícios</h2><p class="hint">Escolha o grupo pelo nome. O ID fica interno.</p>${this._data.exercises.map((item) => `<div class="row"><span>${esc(item.name)}<small class="muted"> · ${esc(this.groupName(item.muscle_group_id))}</small></span><button class="secondary" data-delete="exercises" data-id="${esc(item.id)}">Excluir</button></div>`).join("") || "<p>Nenhum exercício.</p>"}<details><summary>Adicionar exercício</summary><form data-collection="exercises"><label>Nome<input name="name" required></label><label>Grupo muscular<select name="muscle_group_id" required>${this.groupOptions()}</select></label><button>Salvar exercício</button></form></details></section>
  ...
  async handleSubmit(event) {
    ...
    await this._hass.callWS(message); this._editingWorkoutId = null; form.reset(); await this.load();
  }
