...
class FitFlowPanel extends HTMLElement {
  ...
  renderShell() {
    this.innerHTML = `<style>
      ...
      .app { max-width:1180px; margin:auto; } .top { display:flex; align-items:center; justify-content:space-between; gap:24px; padding-bottom:20px; margin-bottom:28px; border-bottom:1px solid var(--divider-color); }
      .brand { display:flex; align-items:center; gap:12px; min-width:0; } .brand-icon { flex:0 0 auto; color:var(--primary-color); --mdc-icon-size:34px; } .brand h1 { margin-bottom:4px; } h1,h2,h3 { margin:0 0 8px; } p { color:var(--secondary-text-color); } .tabs { display:flex; gap:4px; flex-wrap:wrap; padding:4px; border:1px solid var(--divider-color); border-radius:var(--ha-card-border-radius,12px); background:var(--secondary-background-color); }
      ...
      button:hover { filter:brightness(1.08); transform:translateY(-1px); } .tabs button { background:transparent; color:var(--secondary-text-color); border-radius:var(--ha-control-button-border-radius,20px); } .tabs button[aria-current="page"] { background:var(--primary-color); color:var(--text-primary-color,var(--primary-text-color)); } button.secondary { background:var(--secondary-background-color); color:var(--primary-text-color); } .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(300px,1fr)); gap:18px; }
      ...
      .row:last-child { border:0; } .actions { display:flex; gap:8px; flex-wrap:wrap; } label { display:block; margin:10px 0 5px; color:var(--secondary-text-color); } .feedback { margin-bottom:18px; padding:12px 14px; border:1px solid var(--error-color); border-radius:var(--ha-card-border-radius,12px); color:var(--error-color); background:var(--card-background-color); }
      ...
      .date-heading { margin:28px 0 10px; font-size:1.05em; color:var(--primary-text-color); } .history-item { display:block; padding:16px 0; border-bottom:1px solid var(--divider-color); } .history-item:last-child { border:0; } .history-top { display:flex; justify-content:space-between; align-items:flex-start; gap:16px; } .history-name { font-size:1.05em; font-weight:600; } .history-meta { margin-top:5px; color:var(--secondary-text-color); font-size:.9em; } .badge { display:inline-block; padding:4px 9px; border-radius:12px; background:var(--secondary-background-color); color:var(--secondary-text-color); font-size:.78em; white-space:nowrap; } .history-details { margin-top:10px; color:var(--secondary-text-color); } .history-details summary { cursor:pointer; color:var(--primary-color); } .exercise-link { display:inline-flex; align-items:center; gap:8px; color:var(--primary-text-color); border:0; padding:0; background:transparent; cursor:pointer; } .exercise-image { width:54px; height:54px; border-radius:8px; object-fit:cover; vertical-align:middle; background:var(--secondary-background-color); } .image-modal[hidden] { display:none; } .image-modal { position:fixed; inset:0; z-index:10; display:grid; place-items:center; padding:24px; } .image-modal-backdrop { position:absolute; inset:0; background:rgba(0,0,0,.72); } .image-modal-card { position:relative; z-index:1; display:flex; flex-direction:column; max-width:min(92vw,1000px); max-height:92vh; padding:18px; border-radius:var(--ha-card-border-radius,12px); background:var(--card-background-color); box-shadow:0 8px 36px rgba(0,0,0,.35); } .image-modal-header { display:flex; align-items:center; justify-content:space-between; gap:16px; margin-bottom:12px; } .image-modal-header h2 { margin:0; } .image-modal-image { display:block; max-width:100%; max-height:calc(92vh - 100px); object-fit:contain; }
    ...
    </style><div class="app"><div class="top"><div class="brand"><ha-icon class="brand-icon" icon="mdi:arm-flex" aria-hidden="true"></ha-icon><div><h1>FitFlow</h1><p>Treinos flexíveis, escolhidos pelo seu histórico.</p></div></div><nav class="tabs" aria-label="Navegação do FitFlow"><button type="button" data-view="today" aria-current="page">Hoje</button><button type="button" data-view="history">Histórico</button><button type="button" data-view="config">Configuração</button></nav></div><div id="feedback" role="status" aria-live="polite"></div><main id="content" aria-live="polite"><div class="card"><p>Carregando FitFlow...</p></div></main></div><div class="image-modal" data-image-modal hidden role="dialog" aria-modal="true" aria-labelledby="image-modal-title"><div class="image-modal-backdrop" data-close-image></div><div class="image-modal-card"><div class="image-modal-header"><h2 id="image-modal-title"></h2><button type="button" class="secondary" data-close-image>Fechar</button></div><img class="image-modal-image" data-image-modal-image alt=""></div></div>`;
  ...
  clearError() { const feedback = this.querySelector("#feedback"); if (feedback) feedback.innerHTML = ""; }
  showError(error) { const feedback = this.querySelector("#feedback"); if (!feedback) return; const raw = String(error?.message || error || "Não foi possível concluir a operação."); const message = raw.includes("still referenced") ? "Não é possível excluir este grupo muscular porque ele ainda é usado por exercícios ou treinos." : raw; feedback.innerHTML = `<div class="feedback" role="alert">${esc(message)}</div>`; }
  ...
  muscleGroupConfigCard() { return `<section class="card config-section"><div class="section-header"><div><h2>1. Grupos musculares</h2><p class="hint">Cadastre as regiões do corpo usadas para organizar exercícios e treinos.</p></div><span class="section-count">${this._data.muscle_groups.length} registro(s)</span></div>${this._data.muscle_groups.map((item) => this._editingMuscleGroupId === item.id ? `<div class="row"><div>${this.muscleGroupEditForm(item)}</div></div>` : `<div class="row"><span>${esc(item.name)}</span><div class="actions"><button class="secondary" data-edit-muscle-group="${esc(item.id)}">Editar</button><button class="secondary" data-delete="muscle_groups" data-id="${esc(item.id)}">Excluir</button></div></div>`).join("") || "<p>Nenhum grupo muscular.</p>"}<details><summary>Adicionar</summary><form data-collection="muscle_groups"><label>Nome<input name="name" required></label><button>Salvar</button></form></details></section>`; }
  muscleGroupEditForm(item) { return `<form data-collection="muscle_groups" data-item-id="${esc(item.id)}"><label>Nome<input name="name" value="${esc(item.name)}" required></label><div class="actions"><button>Salvar alterações</button><button type="button" class="secondary" data-cancel-muscle-group>Cancelar</button></div></form>`; }
  ...
  imageMarkup(item) { const url = String(item.image_url || ""); if (!/^https?:\/\//i.test(url)) return ""; const name = String(item.name || "exercício"); return `<button type="button" class="exercise-link" data-image-preview data-image-url="${esc(url)}" data-image-name="${esc(name)}" title="Ampliar imagem" aria-label="Ampliar imagem de ${esc(name)}"><img class="exercise-image" src="${esc(url)}" alt="Imagem de ${esc(name)}" loading="lazy"></button>`; }
  ...
  workoutEditForm(item) { const requirements = item.requirements || []; return `<form class="edit-workout" data-collection="workouts" data-item-id="${esc(item.id)}"><label>Nome<input name="name" value="${esc(item.name)}" required></label><div data-requirements>${requirements.map((req) => `<div class="req"><select name="requirement_group" required>${this.groupOptions(req.muscle_group_id)}</select><input name="requirement_count" type="number" min="1" value="${Number(req.exercise_count || 1)}" required>${requirements.length > 1 ? '<button type="button" class="secondary" data-remove-requirement aria-label="Remover grupo deste treino">Remover</button>' : ""}</div>`).join("") || `<div class="req"><select name="requirement_group" required>${this.groupOptions()}</select><input name="requirement_count" type="number" min="1" value="1" required></div>`}</div><div class="actions"><button type="button" class="secondary" data-add-requirement>+ Adicionar grupo</button><button>Salvar alterações</button><button type="button" class="secondary" data-cancel-edit>Cancelar</button></div></form>`; }
  ...
  async handleClick(event) {
    ...
    if (preview) { event.preventDefault(); event.stopPropagation(); this.openImagePreview(preview.dataset.imageUrl, preview.dataset.imageName); return; }
    if (event.target.closest("[data-close-image]")) { this.closeImagePreview(); return; }
    ...
    const editMuscleGroup = event.target.closest("[data-edit-muscle-group]");
    if (editMuscleGroup) { this._editingMuscleGroupId = editMuscleGroup.dataset.editMuscleGroup; this.renderContent(); return; }
    ...
    if (event.target.closest("[data-cancel-muscle-group]")) { this._editingMuscleGroupId = null; this.renderContent(); return; }
    const addRequirement = event.target.closest("[data-add-requirement]");
    if (addRequirement) {
      const form = addRequirement.closest("form");
      form.querySelector("[data-requirements]").insertAdjacentHTML("beforeend", `<div class="req"><select name="requirement_group" required>${this.groupOptions()}</select><input name="requirement_count" type="number" min="1" value="1" required><button type="button" class="secondary" data-remove-requirement aria-label="Remover grupo deste treino">Remover</button></div>`);
      return;
    }
    const removeRequirement = event.target.closest("[data-remove-requirement]");
    if (removeRequirement) { const form = removeRequirement.closest("form"); const rows = form?.querySelectorAll(".req"); if (rows?.length > 1) { removeRequirement.closest(".req").remove(); if (rows.length === 2) form.querySelector("[data-remove-requirement]")?.remove(); } return; }
    ...
    const del = event.target.closest("[data-delete]"); if (del && confirm("Excluir este item?")) { del.disabled = true; try { await this._hass.callWS({type:"fit_flow/mutate", collection:del.dataset.delete, item_id:del.dataset.id, delete:true}); await this.load(); } catch (error) { this.showError(error); } finally { del.disabled = false; } }
  }
  async handleSubmit(event) {
    ...
    await this._hass.callWS(message); this._editingWorkoutId = null; this._editingExerciseId = null; this._editingMuscleGroupId = null; form.reset(); await this.load();
  }
  async callSession(operation, extra = {}) { await this._hass.callWS({type:"fit_flow/session", operation, ...extra}); await this.load(); }
  openImagePreview(url, name) { const modal = this.querySelector("[data-image-modal]"), image = this.querySelector("[data-image-modal-image]"), title = this.querySelector("#image-modal-title"); if (!modal || !image || !title) return; title.textContent = name || "Imagem do exercício"; image.src = url; image.alt = `Imagem de ${name || "exercício"}`; modal.hidden = false; modal.querySelector("[data-close-image]")?.focus(); }
  closeImagePreview() { const modal = this.querySelector("[data-image-modal]"), image = this.querySelector("[data-image-modal-image]"); if (!modal) return; modal.hidden = true; if (image) image.removeAttribute("src"); }
}
