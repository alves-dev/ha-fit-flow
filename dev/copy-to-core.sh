#!/bin/sh
# Deploy FitFlow to the local Home Assistant test configuration.

set -eu

readonly ROOT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"
readonly SOURCE_DIR="$ROOT_DIR/custom_components/fit_flow"
readonly HA_CONFIG_DIR="/home/alves-dev/projects/others/core/config"
readonly TARGET_DIR="$HA_CONFIG_DIR/custom_components/fit_flow"
...
echo "Copied FitFlow to $TARGET_DIR."
sh "$START_SCRIPT"
